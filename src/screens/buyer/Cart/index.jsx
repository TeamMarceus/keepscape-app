      ...
      setNewUserCart(
        userCart.cartSellers.map((cart) => ({
          ...
          cartItems: cart.cartItems.map((item) => ({
            ...
            isSelected: false,
            quantity:
              item.quantity > item.totalStock ? item.totalStock : item.quantity,
          })),
        }))
      );
        ...
        <>
          <Card className={styles.Cart_details}>
            ...
            <div className={styles.Cart_details_additionals}>
              <Text colorClass={colorClasses.NEUTRAL['400']}>Unit Price</Text>

              <Text colorClass={colorClasses.NEUTRAL['400']}>Quantity</Text>

              <Text colorClass={colorClasses.NEUTRAL['400']}>Total Price</Text>

              <Text colorClass={colorClasses.NEUTRAL['400']}>Actions</Text>
            </div>
          </Card>
                ...
                <Button
                  ...
                  disabled={
                    ...
                    (outOfStockItems.reduce(
                      (total, item) =>
                        total +
                        item.cartItems.filter((cart) => cart.isSelected).length,
                      0
                    ) === 0 &&
                      totalPrice === 0)
                  }
                  ...
                  onClick={async () => {
                    ...
                    const availableCartItemIds = newUserCart.reduce(
                      (total, cart) =>
                        total.concat(
                          cart.cartItems
                    ...
                    const outOfStockItemIds = outOfStockItems.reduce(
                      (total, cart) =>
                        total.concat(
                          cart.cartItems
                            .filter((item) => item.isSelected)
                    ...
                    const cartItemsIds =
                      availableCartItemIds.concat(outOfStockItemIds);
                ...
                >
                  Total (
                  {
                    ...
                    newUserCart.reduce(
                      (total, cart) =>
                        total +
                        cart.cartItems.filter((item) => item.isSelected).length,
                      0
                    )
                  }{' '}
                  item
